#!/usr/bin/env python3
# SpectreSpoofer Assistant Bot - Production build (Railway-friendly; env-based config)
...
# ========= LIVE SERVER / AUTH =========
BASE_URL = os.environ.get("BASE_URL", "https://web-production-00cb2.up.railway.app")
...
def send_support_email(user, message):
    try:
        ...
        with smtplib.SMTP_SSL("smtppro.zoho.com", 465) as smtp:
            # Move this to env in production: SMTP_APP_PASSWORD
            smtp.login("team@spectrespoofer.com", os.environ.get("SMTP_APP_PASSWORD", ""))
...
async def spoof(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    hwid = generate_hwid()
    try:
        r = requests.post(f"{BASE_URL}/spoof", json={"key": license_key, "hwid": hwid}, timeout=30)
        data = r.json()
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    license_key = context.args[0]
    try:
        r = requests.post(f"{BASE_URL}/key_stats", json={"key": license_key}, timeout=30)
        data = r.json()
        if r.status_code == 200:
            await update.message.reply_text(
                f"🔐 License Info:\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'manage_plan':
        ...
        try:
            ...
            r = requests.get(f"{BASE_URL}/billing_portal?email={username}@example.com", timeout=30)
            data = r.json()
    ...
    if query.data == 'send_video':
        ...
        try:
            r = requests.get(f"{BASE_URL}/view_keys", headers=AUTH, timeout=30)
            data = r.json()
...
def main():
    ...
    if not SERVER_SECRET:
        logging.warning("SERVER_SECRET not set; protected endpoints will fail.")
